...
Instead of analyzing pixels for glitches, we trace the **genealogical history** of digital media to reveal its original context. By fusing deep reverse-image forensics with semantic reasoning, Paper Trails automatically generates an **evidence board** that exposes when a real image is being hijacked for a false narrative or when a synthetic image has no digital footprint at all. We don’t ask the user to trust a “Fake” label—we give them the historical proof to see the lie for themselves.
