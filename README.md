...
| **Backboard** | Second opinion when Bedrock returns UNVERIFIED | Another **independent model** on borderline cases. Parallel with Gemini so we don’t add latency when the primary verdict is already confident. |
...
app/
  ...
  icon.tsx              # Site icon (favicon / tab icon)
