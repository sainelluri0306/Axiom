...
type PageResultItem = { date: string; link: string; title: string; snippet: string | null; source: string };
type TimelineNode = {
  label: string;
  date: string;
  description: string;
...
export default function TestTextPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const data = (await res.json()) as TextAnalysisBody;
    ...
    <div className="min-h-screen bg-noir-bg text-zinc-100 p-6 font-sans">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <p className="text-sm text-zinc-500">
          Enter a claim to fact-check. The response from{" "}
          <code className="text-zinc-400">/api/analyze-text</code> uses web search, PolitiFact,
          Snopes, and Claude&apos;s analysis. No image input.
        </p>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="claim" className="block text-sm text-zinc-500 mb-1">
              Claim
            </label>
            <input
              id="claim"
              ...
              placeholder="e.g. This is barack obama"
        ...
        {rawResponse !== null && rawResponse.ok && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              {/* Sources: PolitiFact, Snopes, Web */}
              {rawResponse.body.pageResults && rawResponse.body.pageResults.length > 0 && (
                <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                  ...
                  <ul className="space-y-2">
                    {rawResponse.body.pageResults.slice(0, 12).map((p, i) => (
                      <li key={i} className="text-sm">
                        <span className="text-zinc-500 text-xs uppercase mr-2">[{p.source}]</span>
                        <a
                          href={p.link}
                          target="_blank"
                          rel="noopener noreferrer"
